...
const REASONS = [
  ['/assets/image/reason-1.jpg', 'Lower Electricity Bills', 'Shift consumption away from peak tariffs and cut diesel spend — turning energy from a volatile cost into a predictable, managed one.'],
  ['/assets/image/reason-2.jpg', 'Higher Production Uptime', 'Seamless backup keeps critical lines running through grid outages, protecting output, deadlines, and buyer confidence.'],
  ['/assets/image/reason-3.jpg', 'A Smaller Carbon Footprint', 'Cleaner energy use reduces emissions and supports the compliance evidence international buyers increasingly require.'],
];
const WORK = [
    ...
    <>
      ...
      <section className="mx-auto flex max-w-site flex-col gap-20 px-10 py-32 max-[620px]:px-6 max-[620px]:py-[72px]">
        <div className="flex items-end gap-10 max-[1000px]:flex-col max-[1000px]:items-start max-[1000px]:gap-8">
          <h1 className="flex-1 text-[clamp(40px,4.6vw,64px)] font-medium leading-[1.12] tracking-[-1px] text-ink">Industrial Power You Can Count On —<br />Customised to Your Site.</h1>
      ...
      <div className="px-10 pb-10 max-[620px]:px-6">
        <div className="mx-auto flex max-w-site items-start gap-0 rounded-md bg-sand px-10 py-[120px] max-[1000px]:flex-col max-[1000px]:gap-8 max-[620px]:px-6">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-10">
            ...
            <div className="flex gap-5 max-[1000px]:flex-wrap">
              {STATS.map(([n, d]) => (
                <div key={n} className="flex flex-1 flex-col gap-1 rounded-2xl bg-sand-2 px-4 py-5 max-[1000px]:flex-[1_1_45%]">
                  <div className="text-[32px] font-medium leading-10 tracking-[-1px] text-ink">{n}</div>
      ...
      <section className="mx-auto flex max-w-site flex-col gap-12 px-10 py-16 max-[620px]:px-6">
        <div className="flex flex-col gap-10">
          ...
          <div className="flex flex-col gap-4">
            <h2 className="text-[clamp(38px,4.4vw,56px)] font-medium leading-[1.14] tracking-[-1px] text-ink">Purpose-Built for Your Sector</h2>
        ...
        <div className="grid grid-cols-3 gap-10 max-[1000px]:grid-cols-2 max-[620px]:grid-cols-1">
      ...
      {/* Three Reasons */}
      <section className="mx-auto flex max-w-site flex-col gap-[100px] px-10 py-16 max-[620px]:px-6">
        <div className="flex items-end justify-between gap-10 max-[1000px]:flex-col max-[1000px]:items-start max-[1000px]:gap-8">
          <h2 className="max-w-[800px] text-[clamp(40px,4.4vw,56px)] font-medium leading-[1.14] tracking-[-1px] text-ink">Three Reasons Industry<br />Chooses Storage</h2>
        ...
        <div className="flex h-[648px] overflow-hidden rounded-2xl max-[1000px]:h-auto max-[1000px]:flex-col">
          {REASONS.map(([img, t, d]) => (
            <div key={t} className="relative min-w-0 flex-1 overflow-hidden max-[1000px]:h-[420px]">
      ...
      <section className="bg-ink py-32 text-white max-[620px]:py-[72px]">
        <div className="mx-auto flex max-w-site flex-col gap-10 px-10 max-[620px]:px-6">
          <h2 className="text-[clamp(40px,4.4vw,56px)] font-medium leading-[1.14] tracking-[-1px] text-white">How Industry Puts<br />Storage to Work</h2>
          <div className="flex gap-5 max-[1000px]:flex-wrap">
            {WORK.map(([icon, t, d]) => (
      ...
      <section className="px-10 py-16 max-[620px]:px-6">
        <div className="mx-auto flex max-w-site items-start gap-0 max-[1000px]:flex-col max-[1000px]:gap-8">
          ...
          <div className="flex min-w-0 flex-1 flex-col items-start gap-10">
            <h2 className="text-[clamp(40px,4.4vw,56px)] font-medium leading-[1.14] tracking-[-1px] text-ink">Power That Protects Your Export Edge</h2>
